 ...
 * Normalizes model_version and ssot_version for consistent access
 */
export async function loadDashboardConfig() {
  ...
  // Normalize version fields: ensure model_version and ssot_version exist
  // Legacy support: if 'version' exists but model_version doesn't, use it as model_version
  if (!configData.model_version) {
    if (configData.version) {
      configData.model_version = configData.version;
      console.warn('[config-loader] Legacy "version" field found, using as model_version. Please update config to use model_version.');
    } else {
      throw new Error('Configuration missing required field: model_version');
    }
  }
  
  // Ensure ssot_version exists (default to model_version if missing)
  if (!configData.ssot_version) {
    configData.ssot_version = configData.model_version || '1.0.0';
    console.warn('[config-loader] ssot_version not found, defaulting to model_version. Please add ssot_version to config.');
  }
...
export function validateConfig(config) {
  ...
  // Note: 'version' is legacy; we now require 'model_version'
  const requiredFields = ['model_version', 'pillars', 'factors', 'subweights', 'bands'];
  ...
  // Validate model_version format (should be a string like "v1.1")
  if (config.model_version && typeof config.model_version !== 'string') {
    errors.push(`model_version must be a string, got ${typeof config.model_version}`);
  }
 ...
 * Returns normalized config with guaranteed model_version and ssot_version
 */
...
/**
 * Get normalized model_version from config (guaranteed to exist)
 */
...
/**
 * Get normalized ssot_version from config (guaranteed to exist)
 */
export async function getSsotVersion() {
  const config = await getDashboardConfig();
  return config.ssot_version;
}

/**
 * Integrity pack revision stamp (v1.1.1).
 */
export async function getImplementationRevision() {
  const config = await getDashboardConfig();
  return config.implementation_revision || 'etf-sosovalue-vix-cboe-2026-09';
}
