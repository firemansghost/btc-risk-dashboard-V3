...
export default function FactorDetailsDrawer({ 
  ...
  // Scroll to "More details" section when requested
  useEffect(() => {
    if (isOpen && scrollToSection === 'moreDetails' && moreDetailsRef.current && contentRef.current) {
    ...
    <>
      ...
      >
        ...
        <div ref={contentRef} className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {/* More Details - Expanded rows from factor.details */}
          {factor?.details && factor.details.length > 3 && (
            <div ref={moreDetailsRef}>
              <h3 className="text-sm font-semibold text-gray-900 mb-2">More details</h3>
