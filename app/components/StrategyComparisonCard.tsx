...
interface ComparisonData {
  ...
  metadata?: {
    generatedAt?: string;
    schedule?: string;
    bandDerivation?: string;
    exploratoryStrategies?: string[];
  };
  strategies: { [key: string]: StrategyData };
  exploratory?: {
    note?: string;
    valueAveraging?: StrategyData;
  };
  rankings: {
      ...
      <div className="p-6">
        {activeTab === 'overview' && (
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <h5
                    ...
                    title="Ranking by mean/std of period returns between trades (Sharpe-like); not annualized Sharpe."
                  >
                    By Sharpe-like ratio (trade intervals)
                  </h5>
                  <div className="space-y-2">
            ...
            {comparisonData.exploratory?.valueAveraging && (
              <div className="rounded-lg border border-dashed border-amber-300 bg-amber-50/80 p-4">
                <h4 className="text-sm font-medium text-amber-950 mb-2">Exploratory: Value Averaging</h4>
                <p className="text-xs text-amber-900 mb-2">{comparisonData.exploratory.note}</p>
                <div className="text-sm text-amber-950">
                  Return:{' '}
                  <span className="font-semibold">
                    {(comparisonData.exploratory.valueAveraging.totalReturn * 100).toFixed(2)}%
                  </span>
                  {' · '}
                  Trades: {comparisonData.exploratory.valueAveraging.trades.length} · Not part of official two-strategy rankings above.
                </div>
              </div>
            )}
          </div>
          ...
          <div className="space-y-6">
            {comparisonData.exploratory?.valueAveraging && (
              <p className="text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded p-2">
                Details below list official strategies first. Value averaging appears under exploratory (same artifact file).
              </p>
            )}
            {Object.entries(comparisonData.strategies).map(([name, strategy]) => (
