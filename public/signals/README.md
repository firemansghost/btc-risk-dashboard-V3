...
Forward structured series live in `v2/` and are written from `metrics` on factor results. Missing metrics are blank, never coerced to `0`. Daily ETL ETF zero-cross alerts and leftover signal-history readers consume `v2/`, not these frozen files.
